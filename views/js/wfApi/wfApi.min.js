...
if(environment.method){if(/^get|post$/i.test(environment.method)){this.sourceService.method=environment.method;}}}}}};this.retrieveContext=function(){switch(this.sourceService.type){case'manual':this.registry=this.sourceService.data;break;case'sync':try{var response=$.parseJSON($.ajax({async:false,url:this.sourceService.url,data:this.sourceService.params,type:this.sourceService.method,dataType:this.sourceService.format}).responseText);if($.isPlainObject(response)||$.isArray(response)){this.registry=response;}}
catch(jsonException){}
