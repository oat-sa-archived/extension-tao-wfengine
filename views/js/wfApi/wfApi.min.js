...
if(environment.method){if(/^get|post$/i.test(environment.method)){this.sourceService.method=environment.method;}}}}}};this.retrieveContext=function(){switch(this.sourceService.type){case'manual':this.registry=this.sourceService.data;break;case'sync':var response=$.parseJSON($.ajax({async:false,url:this.sourceService.url,data:this.sourceService.params,type:this.sourceService.method,dataType:this.sourceService.format}).responseText);if(response.length>0){this.registry=response;}
break;case'async':$.ajax({async:false,url:this.sourceService.url,data:this.sourceService.params,type:this.sourceService.method,dataType:this.sourceService.format,success:function(received){this.registry=received;}});break;}};this.initDestinationService=function(environment){if($.isPlainObject(environment)){if(environment.type){if($.inArray(environment.type,['sync','async'])>-1){this.destinationService.type=environment.type;}}
